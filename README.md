...
**성격**: 활동적이고 계획적인 성향. 친구들과 즉흥적인 만남도 즐기지만, 약속이나 여행에는 어느 정도의 계획이 필요하다고 생각함.

**목표**: 연인과의 약속을 알차고 즐겁게 보내고 싶고, 코스나 일정 계획을 자동으로 추천받아 스트레스를 줄이고 싶다.

**니즈**:

- 약속 코스를 시간대별로 추천해주는 AI
...
**목표**: 연인과의 약속을 위해 코스와 일정을 자동으로 추천받기

  

**시나리오**:

민수는 여자친구와 이번 주말에 약속이 있다. 하지만 바쁜 과제와 아르바이트 일정 때문에 자세한 약속 코스를 짤 시간이 없다.
...
AI는 몇 초 만에 **오전 11시부터 오후 7시까지의 약속 코스**를 10분 단위로 자동으로 구성한다.
...
민수는 앱 내에서 식당 예약까지 완료하고, 일정대로 약속을 즐긴다.
...
"연인으로서, 저는 최소한의 정보를 입력해서 자동으로 약속 일정을 생성받고 싶습니다. 그래서 쉽게 계획을 세우고 스트레스를 줄일 수 있습니다."
...
- 조건: 지역(예: 홍대), 인원 수(1~4명), 일정 종류(맛집, 휴식, 약속 등)
...
1. **약속 자동 추천 (목업 기반)**
   - 인원 수, 지역, 시작/종료 시간, 이동수단 등 최소 정보 입력만으로 추천 일정 생성
   - 실제 AI가 아닌, 프론트엔드에서 미리 정의된 추천 로직(목업 데이터) 사용

2. **장소 커스터마이즈**
   - 사용자가 원하는 장소(카페, 식당 등) 직접 추가/제거/순서변경 가능
   - 각 장소별 머무는 시간 직접 수정 가능

3. **일정 미리보기 및 타임라인**
   - 추천 일정은 타임라인(세부 일정)과 지도 기반 UI로 한눈에 확인
   - 각 일정별 장소명, 활동명, 시간, 이동시간 등 상세 정보 제공

4. **일정 공유**
   - '공유하기' 버튼 클릭 시, 현재 페이지 URL을 복사하여 타인과 공유 가능
   - (실제 서버 기반 공유/QR코드 기능은 미구현)

5. **예약 연동**
   - (아직 미구현, 추후 식당/카페 등 예약 버튼 추가 가능)

---

## 구현 범위 (MVP)

- **AI/서버 연동 없이 프론트엔드 목업 데이터 기반 추천**
- **장소 커스터마이즈(추가/삭제/순서변경/머무는 시간 수정)**
- **타임라인 및 지도 기반 일정 미리보기**
- **공유(링크 복사) 기능**
- **예약/QR코드/실제 AI 추천 등은 미구현**

---

## 사용자 흐름

1. **기본 정보 입력**: 인원 수, 지역, 시간, 이동수단 등 입력
2. **장소 선택 및 커스터마이즈**: 원하는 장소 추가/삭제/순서변경, 머무는 시간 수정
3. **추천 일정 확인**: 타임라인 및 지도에서 전체 일정 확인
4. **일정 공유**: '공유하기' 버튼으로 URL 복사
...
- 실제 AI/서버 연동 추천
- 예약 버튼 및 외부 예약 서비스 연동
- QR코드 공유
- 일정 종류(맛집, 휴식 등) 직접 선택 UI
- 장소별 상세 정보(영업시간, 리뷰 등) 표시

---

## 테스트용 안내

- 현재 추천 일정은 입력값에 따라 목업 데이터에서 생성됩니다.
- 실제 장소 정보, 이동시간 등은 예시 데이터임을 참고해주세요.
